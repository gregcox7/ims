# (PART) Foundations for inference {.unnumbered}
...
If the null hypothesis and the data notably disagree, then we will *reject* the null hypothesis and conclude that chance alone does not explain any observed differences.  The claim that observed differences are not due to chance is the alternative hypothesis.
...
We will define a **success**\index{success} in this study as a student who chooses not to buy the video.[^foundations-randomization-7]
...
[^foundations-randomization-7]: Success is often defined in a study as the outcome of interest, and a "success" may or may not actually be a positive outcome.
...
The **p-value**\index{hypothesis testing!p-value} is the probability of observing data that is at least as extreme as our current dataset, if the null hypothesis were true.
...
inference_method_summary_table %>%
  ...
  kbl(linesep = "\\addlinespace", booktabs = TRUE, caption = "Summary of randomization as an inferential statistical method.", 
...
```{r exercises-11, child = "exercises/11-ex-foundations-randomization.Rmd"}
```
:::
