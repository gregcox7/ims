...
county %>%
  ...
  kbl(linesep = "", booktabs = TRUE, 
      caption = caption_helper("Six observations and six variables from the `county` dataset."), format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"))
```
...
county_var_def %>%
  kbl(linesep = "", booktabs = TRUE, caption = caption_helper("Variables and their descriptions for the `county` dataset."), 
      col.names = c("Variable", "Description")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = TRUE) %>%
  column_spec(1, monospace = TRUE) %>%
...
```{r county-multi-unit-homeownership, fig.cap = "A scatterplot of homeownership versus the percent of housing units that are in multi-unit structures for US counties. The highlighted dot represents Chattahoochee County, Georgia, which has a multi-unit rate of 39.4\\% and a homeownership rate of 31.3\\%."}
...
```{r county-pop-change-med-hh-income, fig.cap = "A scatterplot showing population change against median household income. Owsley County of Kentucky, is highlighted, which lost 3.63\\% of its population from 2010 to 2017 and had median household income of \\$22,736."}
