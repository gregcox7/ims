...
::: {.guidedpractice data-latex=""}
Using the histogram of bootstrapped difference in means, estimate the standard error of the differences in sample means, $\bar{x}_{ESC} - \bar{x}_{Control}.$[^inference-two-means-3]
:::

[^inference-two-means-3]: The boundaries of the confidence interval are around 4.5 and 12.  Since the confidence interval roughly spans 2 standard errors on *either* side of the mean, we can divide the width of the confidence interval by four to get a rough estimate of the standard error: $\frac{12 - 4.5}{4} = 1.875$.
...
xbar_nonsmoker  <- births14_sample_stats %>% filter(habit == "nonsmoker") %>% pull(Mean)
xbar_smoker     <- births14_sample_stats %>% filter(habit == "smoker") %>% pull(Mean)
xbar_difference <- xbar_nonsmoker - xbar_smoker
sd_nonsmoker    <- births14_sample_stats %>% filter(habit == "nonsmoker") %>% pull(SD)
sd_smoker       <- births14_sample_stats %>% filter(habit == "smoker") %>% pull(SD)
...
s_pool <- sqrt((sd_smoker^2 * (n_smoker - 1) + sd_nonsmoker^2 * (n_nonsmoker - 1)) / (n_smoker + n_nonsmoker - 2))
se_difference <- s_pool * sqrt(1 / n_smoker + 1 / n_nonsmoker)
t_difference    <- xbar_difference / se_difference
# df_difference   <- min(n_nonsmoker-1, n_smoker-1)
df_difference <- n_nonsmoker + n_smoker - 2
one_tail_area <- pt(q = abs(t_difference), df = df_difference, lower.tail = FALSE)
two_tail_area <- 2 * one_tail_area
...
where $s_{\textit{pool}}$ is the *pooled* standard deviation between the two groups:
...
```{r include=FALSE}
terms_chp_20 <- c(terms_chp_20, "T score", "t-test", "pooled standard deviation")
```

Like the normality condition, "similar variability"^[This condition is also called "homogeneity of variance".] is rather vague, so we introduce a rule of thumb.

::: {.important data-latex=""}
**General rule for performing the similar variability check.**
...
[^inference-two-means-5]: First compute the pooled standard deviation: $s_{\textit{pool}} = \sqrt{\frac{(n_n - 1) s^2_n + (n_s - 1) s^2_s}{n_n + n_s - 2}}\\ = \sqrt{\frac{(`r n_nonsmoker` - 1) `r sd_nonsmoker`^2 + (`r n_smoker` - 1) `r sd_smoker`^2}{`r n_nonsmoker` + `r n_smoker` - 2}} = `r s_pool`$.
  Then use $s_{\textit{pool}}$ to find the standard error: $SE(\bar{x}_{n} - \bar{x}_{s}) = s_{\textit{pool}} \sqrt{1 / n_{n} + 1 / n_{s}}\\ = `r s_pool` \sqrt{1 / `r n_nonsmoker` + 1 / `r n_smoker`} = `r se_difference`$.
...
The degrees of freedom are $df = n_n + n_s - 2 = `r n_nonsmoker` + `r n_smoker` - 2 = `r df_difference`$.
...
-   *Similar variability*. The samples should have standard deviations that are not too different.
...
The margin of error is $t^\star_{df} \times s_{\textit{pool}} \sqrt{\frac{1}{n_1} + \frac{1}{n_2}}$ where $t^\star_{df}$ is calculated from a specified percentile on the t-distribution with *df* degrees of freedom.
:::
