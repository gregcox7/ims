...
The first step in conducting research is to specify a **research question**.  Doing this in a clear and thoughtful way is necessary to identify what subjects or cases should be studied and what variables should be observed.

Consider the following research question:

> Do typical adults remember items at the end of a list better than those from earlier in a list?

How do we decide what variables to observe in order to address this research question?  Notice that this research question is about the relationship between two concepts: *memory quality* and *time within a list*.

::: {.guidedpractice data-latex=""}
How might we measure "time within a list"?  What kind of variable would this be?[^data-design-op1]
:::

[^data-design-op1]: There are actually a few ways we could measure time within a list, and different ways result in different types of variables.
...
Unlike time within a list, "memory quality" is not something that can be directly observed.  Memory quality is a **theoretical construct**.  A theoretical construct is something that probably exists but that cannot be directly observed.  Many psychological concepts are theoretical constructs, including things like "happiness", "friendship", "quality of life", or "expertise".

Because a theoretical construct cannot be directly observed, we have to find a way to observe it *indirectly*.  Translating a theoretical construct into something we can observe is called **operationalization**.  We "operationalize" a theoretical construct in terms of something we can observe.  The resulting variable then becomes part of our data.

```{r include=FALSE}
terms_chp_2 <- c("theoretical construct", "operationalization")
```

::: {.workedexample data-latex=""}
How can we operationalize the theoretical construct of *memory quality*?
...
Because there are often multiple ways to operationalize a theoretical construct, we must be concerned with **construct validity**.  Construct validity refers to how well any given operationalization reflects the theoretical construct we are actually interested in.  Different ways of operationalizing a construct may be more valid than others.  Unfortunately, because theoretical constructs cannot be directly observed (by definition), there is no way to know for sure how much construct validity an operationalization has.
...
```{r include=FALSE}
terms_chp_2 <- c("construct validity")
```
...
```{r mn-winter, out.width="35%", fig.cap='In February 2010, some media pundits cited one large snowstorm as evidence against global warming. As comedian Jon Stewart pointed out, "It is one storm, in one region, of one country."', fig.alt = "Photograph shows a city street covered in snow and ice."}
...
```{r pop-to-sample, fig.cap="In this graphic, 10 graduates are randomly selected from the population to be included in the sample.", fig.asp = 0.5, fig.width = 8, out.width = "70%", fig.alt = "A large circle contains many dots which indicate all the graduates.  A smaller circle contains a few of the dots (i.e., graduates) which have been randomly selected from the larger circle."}
set.seed(1234)
...
```{r pop-to-sub-sample-graduates, fig.cap = "Asked to pick a sample of graduates, a nutrition major might inadvertently pick a disproportionate number of graduates from health-related majors.", fig.asp = 0.5, fig.width = 8, out.width = "70%", fig.alt = "A large circle contains many dots which indicate all the graduates, but some of the dots have been greyed out where others are dark dots from which the sample is taken.  A smaller circle contains a few of the dots (i.e., graduates) which have been selected from the biased group of dark dots in the large circle."}
par_og <- par(no.readonly = TRUE) # save original par
par(mar = rep(0, 4))
plot(c(0, 2), c(0, 1.1), type = "n", axes = FALSE, xlab = "", ylab = "")
temp <- seq(0, 2 * pi, 2 * pi / 100)
...
```{r survey-sample, fig.cap="Due to the possibility of non-response, survey studies may only reach a certain group within the population. It is difficult, and oftentimes impossible, to completely fix this problem.", fig.asp = 0.5, fig.width = 8, out.width = "70%", fig.alt = "A large circle contains many dots which indicate the population of interest, but some of the dots have been greyed out where others are dark dots from which the sample is taken (where the grey dots are potentially due to non-response bias).  A smaller circle contains a few of the dots which have been selected from the group of dark dots in the large circle who were individuals willing to respond to the survey."}
par_og <- par(no.readonly = TRUE) # save original par
par(mar = rep(0, 4))
plot(c(0, 2),
     c(0, 1.1),
...
<!-- ```{r simple-stratified, fig.cap="Examples of simple random and stratified sampling. In the top panel, simple random sampling was used to randomly select the 18 cases (denoted in red). In the bottom panel, stratified sampling was used: cases were first grouped into strata, then simple random sampling was employed to randomly select 3 cases within each stratum.", fig.width = 10, out.width = "100%", fig.alt = "The top box shows a population of dots (i.e., individuals) where a handful of the dots have been sampled randomly.  The bottom box shows the same population of dots but grouped in such a way that there are six strata.  From each stratum three dots (i.e., individuals) are randomly selected."} -->
...
**Simple random sampling** is probably the most intuitive form of random sampling and the only type that we will focus on in this book as it is the basis for the statistical methods we will cover.
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2, "simple random sampling")
```
...
<!-- ```{r include=FALSE} -->
<!-- terms_chp_2 <- c(terms_chp_2, "stratified sampling", "strata") -->
<!-- ``` -->
...
<!-- ```{r cluster-multistage, fig.cap="Examples of cluster and multistage sampling. In the top panel, cluster sampling was used: data were binned into nine clusters, three of these clusters were sampled, and all observations within these three clusters were included in the sample. In the bottom panel, multistage sampling was used, which differs from cluster sampling only in that we randomly select a subset of each cluster to be included in the sample rather than measuring every case in each sampled cluster.", fig.width = 10, out.width = "100%", fig.alt = "In the top figure, dots are grouped into clusters, three clusters are selected, and every dot (i.e., all individuals) from each of the three clusters are sampled.  In the bottom figure, dots are again grouped into clusters and three clusters are selected.  However, random sampling is applied so that a random sample from each of the three selected clusters is taken."} -->
...
<!-- We learn that there are 30 villages in that part of the Indonesian jungle, each more or less similar to the next, but the distances between the villages are substantial. -->
...
::: {.important data-latex=""}
**Randomness, chance, variability, and uncertainty.**
...
1.  **Controlling.** Researchers assign treatments to cases, and they do their best to **control** any other differences in the groups[^data-design-3]. For example, when patients take a drug in pill form, some patients take the pill with only a sip of water while others may have it with an entire glass of water. To control for the effect of water consumption, a doctor may instruct every patient to drink a 12 ounce glass of water with the pill.

[^data-design-3]: This is a different concept than a *control group*, which we discuss in the second principle and in Section \@ref(reducing-bias-human-experiments).
...
2.  **Randomization.** Researchers randomize patients into treatment groups to account for variables that cannot be controlled.
    ...
    In this example dietary habit is a **confounding variable**[^data-design-4], which is defined as a variable that is associated with both the explanatory and response variables.
...
Study volunteers[^data-design-5] were randomly placed into two study groups.
...
[^data-design-5]: Human subjects are often called **patients**, **volunteers**, or **study participants**.
...
To guard against this bias, which again has been found to have a measurable effect in some instances, most modern studies employ a **double-blind** setup where doctors or researchers who interact with patients are, just like the patients, unaware of who is or is not receiving the treatment.[^data-design-6]

[^data-design-6]: There are always some researchers involved in the study who do know which patients are receiving which treatment.
...
Was it double-blinded?[^data-design-7]
:::

[^data-design-7]: The researchers assigned the patients into their treatment groups, so this study was an experiment.
...
If so, what would that placebo have looked like?[^data-design-8]
:::

[^data-design-8]: Ultimately, can we make patients think they got treated from a surgery?
...
Does this mean sunscreen *causes* skin cancer?[^data-design-9]
:::
...
```{r sun-causes-cancer, out.width = "60%", fig.asp = 0.4, fig.alt = "Three boxes are shown in a triangle arrangement representing: sun exposure, using sunscreen, and skin cancer.  A solid arrow connects sun exposure as a causal mechanism to using sunscreen; a solid arrow also connects sun exposure as a causal mechanism to skin cancer.  A questioning arrow indicates that the causal effect of using sunscreen on skin cancer is unknown."}
par_og <- par(no.readonly = TRUE) # save original par
par(mar = rep(0, 4))
plot(c(-0.05, 1.2),
     c(0.39, 1),
...
Suggest a variable that might explain the negative relationship.[^data-design-10]
:::

[^data-design-10]: Answers will vary.
...
```{r exercises-02, child = "exercises/02-ex-data-design.Rmd"}
```
:::
