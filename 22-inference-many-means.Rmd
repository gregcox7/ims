...
Instead, we should apply a single *omnibus* test to check whether there is evidence that at least one pair groups are in fact different, and this is where **ANOVA** saves the day.
...
This question is different from earlier testing procedures since we will *simultaneously* consider many groups, and evaluate whether their sample means collectively differ more than we would expect from natural variation.
...
$$
where $SSG$ is called the **sum of squares between groups** and $n_{i}$ is the sample size of group $i.$  Software is typically used to perform these computations.
...
There are two equivalent ways to find $MSE$, though again these calculations are typically carried out using software.  Again let $\bar{x}$ represent the mean of outcomes across all groups.  Then the **sum of squares total (**$SST)$ is computed as
$$
SST = \sum_{i=1}^{n} \left(x_{i} - \bar{x}\right)^2
$$
where the sum is over all observations in the dataset.  Then we compute the **sum of squared errors** $(SSE)$ by subtraction $SSE = SST - SSG$.  You can also find the $SSE$ as the sum of the sample variances of each group, weighted by the degrees of freedom in each group:
$$
SSE = \sum_{i=1}^{k} (n_i - 1) s_i^2
$$
Either way you find $SSE$, the $MSE$ is the standardized form of $SSE: MSE = \frac{1}{df_{E}}SSE.$  Additional details of the computations of the $MSE$ and a link to an extra online section for ANOVA calculations are provided in the footnote.[^inference-many-means-4]
...
This is a problem that arises whenever conducting multiple hypothesis tests using the same data, and is known as the problem of **multiple comparisons**.
...
## Mathematical model for comparing many means {#mathANOVA}
...
However, the ANOVA test does not provide a mechanism for knowing *which* group is driving the differences (see Section \@ref(anova-post-hoc)).
...
## Post-hoc tests {#anova-post-hoc}

While ANOVA can test whether there is *any* difference between group means, it cannot tell us *which* groups differ from one another.
...
As we saw at the end of Section \@ref(randANOVA), this is a problem of **multiple comparisons**\index{multiple comparisons}.
...
```{r include=FALSE}
terms_chp_22 <- c(terms_chp_22, "post-hoc tests", "multiple comparisons", "Bonferroni correction")
```
