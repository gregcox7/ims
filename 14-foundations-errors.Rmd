...
```{r include=FALSE}
terms_chp_14 <- c("type 1 error", "type 2 error")
```
...
```{r include=FALSE}
terms_chp_14 <- c(terms_chp_14, "significance level")
```
...
Also, provide a point estimate $(\hat{p}_T - \hat{p}_C)$ for the true difference in population survival proportions across the two groups: $p_T - p_C.$[^foundations-errors-3]
:::

[^foundations-errors-3]: Observed control survival rate: $\hat{p}_C = \frac{11}{50} = 0.22.$ Treatment survival rate: $\hat{p}_T = \frac{14}{40} = 0.35.$ Observed difference: $\hat{p}_T - \hat{p}_C = 0.35 - 0.22 = 0.13.$
...
```{r exercises-14, child = "exercises/14-ex-foundations-errors.Rmd"}
```
:::
