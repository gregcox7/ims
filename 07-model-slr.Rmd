...
Linear models can be used to predict the value of a response variable based on the value of an explanatory variable.
Linear models can also be used to evaluate whether there is a linear relationship between a numerical variable on the horizontal axis and the average of the numerical variable on the vertical axis.
:::
...
When we use $x$ to predict $y,$ we call $x$ the **explanatory** variable and we call $y$ the **response** variable.
We also often drop the $e$ term when writing down the model since our main focus is often on the prediction of the average response.

```{r include=FALSE}
terms_chp_7 <- c("explanatory variable", "response variable")
```
...
In this example, we will use the total length as the explanatory variable, $x,$ to predict a possum's head length, $y.$ We could fit the linear relationship by eye, as in Figure \@ref(fig:scattHeadLTotalLLine).
...
The residuals are plotted with their predicted response variable value as the horizontal coordinate, and the vertical coordinate as the residual.
...
The first option that may come to mind is to minimize the sum of the absolute values of the residuals:
...
The slope describes the estimated difference in the predicted average response $y$ if the explanatory variable $x$ increased by one unit.
The intercept describes the average response $y$ if $x = 0$ *and* the linear model is valid all the way to $x = 0$ (values of $x = 0$ are not observed or relevant in many applications).
:::
...
where $r$ is the correlation between the two variables, and $s_x$ and $s_y$ are the sample standard deviations of the explanatory and response variables, respectively.

2.  If $\bar{x}$ is the sample mean of the explanatory variable and $\bar{y}$ is the sample mean of the response variable, then the point $(\bar{x}, \bar{y})$ falls on the least squares line.
...
The $R^2$ of a linear model describes the amount of variation in the response variable that is explained by the least squares line.
For example, consider the Elmhurst data, shown in Figure \@ref(fig:elmhurstScatterWLine).
The variance of the response variable, aid received, is about $s_{aid}^2 \approx 29.8$ million (calculated from the data, some of which is shown in Table \@ref(tab:elmhurst-data)).
...
In short, there was a reduction of $$\frac{s_{aid}^2 - s_{_{RES}}^2}{s_{aid}^2}
  ...
  = \frac{7500}{29800}
  \approx 0.25,$$ or about 25%, of the response variable's variation by using information about family income for predicting aid using a linear model.
It turns out that $R^2$ corresponds exactly to the squared value of the correlation:

$$r = -0.499 \rightarrow R^2 = 0.25$$

::: {.guidedpractice data-latex=""}
If a linear model has a very strong negative relationship with a correlation of -0.97, how much of the variation in the response variable is explained by the explanatory variable?[^model-slr-8]
:::

[^model-slr-8]: About $R^2 = (-0.97)^2 = 0.94$ or 94% of the variation in the response variable is explained by the linear model.
...
This statistic is called the **coefficient of determination**, and it measures the proportion of variation in the response variable, $y,$ that can be explained by the linear model with explanatory variable $x.$
:::
...
[^model-slr-10]: $SST$ can be calculated by finding the sample variance of the response variable, $s^2$ and multiplying by $n-1.$

### Categorical explanatory variables with two levels {#categorical-predictor-two-levels}

Categorical variables are also useful in predicting the average of a response variable.
Here we consider a categorical explanatory variable with two levels (recall that a *level* is the same as a *category*).
...
::: {.important data-latex=""}
**Interpreting model estimates for categorical explanatory variables.**
...
However, when the explanatory variable is binary, the coefficient estimates ($b_0$ and $b_1$) are directly interpretable with respect to the dataset at hand.
...
```{r outlier-plots-1, fig.cap = "Three plots, each with a least squares line and corresponding residual plot. Each dataset has at least one outlier.", out.width = "100%", fig.width = 8, fig.asp = 0.6}
...
```{r outlier-plots-2, fig.cap="Three plots, each with a least squares line and residual plot. All datasets have at least one outlier.", out.width="100%"}
...
If one of these high leverage points does appear to actually invoke its influence on the slope of the line -- as in Plots C, D, and E of Figures \@ref(fig:outlier-plots-1) and \@ref(fig:outlier-plots-2) -- then we call it an **influential point**.
...
Correlation is a measure of the strength and direction of the linear relationship of two variables, without specifying which variable is the explanatory and which is the response.
...
```{r exercises-07, child = "exercises/07-ex-model-slr.Rmd"}
```
:::
